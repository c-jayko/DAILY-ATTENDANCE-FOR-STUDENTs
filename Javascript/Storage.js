...
function ModifyItem() {
	...
	document.forms.ListOfDebt.commend.value = JSON.parse(localStorage.getItem(name))[0];
	document.forms.ListOfDebt.borrowed.value = JSON.parse(localStorage.getItem(name))[1];
	document.forms.ListOfDebt.payment.value = JSON.parse(localStorage.getItem(name))[2];
	doShowAll();
}
...
function doShowAll() {
	if (CheckBrowser()) {
		var key = "";
		var list = "<tr><th>Full name</th><th>Time In</th><th>Date</th><th>Time Out</th></tr>\n";
		var i = 0;
		for (i = 0; i <= localStorage.length - 1; i++) {
			key = localStorage.key(i);
			list += "<tr><td>" + key + "</td>\n<td>"
					+ JSON.parse(localStorage.getItem(key))[0]
					+ "</td><td>"+ JSON.parse(localStorage.getItem(key))[1] + "</td><td>"+ JSON.parse(localStorage.getItem(key))[2] +"</td></tr>\n";
		}
		if (list == "<tr><th>Full name</th><th>Time In</th><th>Date</th><th>Time Out</th></tr>\n") {
